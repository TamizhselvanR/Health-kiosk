...
sheets = client.open('kioskdata').sheet1
...
def first_page():
    ...
    def start():
        ...
        temp_Database["living_area"] = living_status.get()
    ...
    faq_but = Button(root, text="Boctor(Bot + Doctor)", font=entry_age, bg="green", command=faq_bot)
    faq_but.pack()

    gap18 = Label(root, bg='grey')
    gap18.config(font=('bold', 1))
    gap18.pack(fill=X)

    gap19 = Label(root, bg='grey')
    gap19.config(font=('bold', 1))
    gap19.pack(fill=X)
...
def third_page(sec_root):
    ...
    def send_data_to_sheets(db):
        data = sheets.get_all_records()

        if(db['ever_married'] == 'Yes'):
            marital_status = 'Married'
        else:
            marital_status = 'Unmarried'
        print(db)
        row = [db['name'], db['age'], db['gender'], marital_status, db['work_type'], db['living_area'], db['height'],
                      db['weight'], db['bmi_status'], db['heart_rate'], db['blood_oxygen_level'], db['temp']]
        sheets.insert_row(row, len(data)+2)

    def restart():
        third_root.destroy()
        send_data_to_sheets(temp_Database)
